...
const CartItem = ({item}) => {
  const {image, id, price , title, amount} = item;
  const { removeFromCart, increaseAmount, decreaseAmount } =useContext(CartContext)
  
  return (
     <div className='flex gap-x-4 py2 lg:px-6 border-b border-gray-200 w-full font-light text-gray-500'>
        ...
        <div className='flex flex-col w-full'>
          ...
          <div className='flex justify-between mb-2 '>
            ...
            <div onClick={()=> removeFromCart(id)} className='text-xl cursor-pointer '>
              <IoMdClose className='text-gray-500 hover:tere500  transition'/>
            </div>
          </div>
          <div className=' flex gap-x-2 h-[36px] text-sm'>
            {/* qty */}
             <div className='flex flex-1 max-w-[100px]  items-center h-full border text-primary font-medium  '>
               ...
               <div className='h-full  flex justify-center items-center px2 '>{amount}</div>
               <div onClick={()=>increaseAmount(id)} className='flex-1 h-full flex justify-center items-center cursor-pointer ' ><IoMdAdd/> </div>
            </div>
            {/* item price */}
             <div className='fle1 flex items-center justify-around '>$ {price}</div>
             {/* final price */}
